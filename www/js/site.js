...
$('#marca').on('change',function(e){
    ...
    $.get('/get-modelo?marca_id=' + marca_id, function(data){
        $('#modelo').append('<option id="0" value="0" data-value="0">Selecione um Modelo</option>');
...
$('#modelo').on('change',function(e){
    ...
    $.get('/get-ano?modelo_id=' + modelo_id, function(data){
        $('#ano').append('<option id="0" value="0" data-value="0">Selecione um Ano</option>');

        $.each(data, function(index, anoObj){
            if(!(anoObj.ano == '3200'))
                $('#ano').append('<option id="'+anoObj.id_ano+'" value="'+anoObj.ano+'" data-value="'+anoObj.valor+'">'+anoObj.ano+'</option>');
        });
...
$('#ano').on('change',function(e){
    $('#valor').empty();
    var variavel = $(this).find('option:selected').attr('data-value');
    $('#valor').attr('value',variavel);
});